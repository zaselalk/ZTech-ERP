...
import { Table, Button, Modal, Form, Select, InputNumber, message, AutoComplete, Steps, Card, Typography, Row, Col, Divider } from 'antd';
...
// Separate Receipt Component for clarity and printing
const Receipt = ({ sale, onDone }) => {
  ...
  return (
    <Modal title="Sale Successful" visible={true} onCancel={onDone} footer={[
...
// Sub-component for the payment form to correctly handle dynamic fields
...
const Sales = () => {
  ...
  const fetchSales = async () => { try { const res = await fetch(`${API_URL}/sales`); setSales(await res.json()); } catch (e) { message.error('Failed to fetch sales'); } };
  ...
  return (
    <div>
      <Button type="primary" onClick={() => setIsModalVisible(true)} style={{ marginBottom: 16 }}>New Sale (POS)</Button>
      ...
      {isModalVisible && (
        <Modal 
            ...
            footer={[
                ...
                <Button key="cancel" onClick={resetPOS}>Cancel</Button>,
                currentStep === 0 ? 
      ...
      {completedSale && <Receipt sale={completedSale} onDone={resetPOS} />}
    </div>
  );
};
