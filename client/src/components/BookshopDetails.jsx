...
const BookshopDetails = () => {
  ...
  useEffect(() => {
    const fetchBookshopDetails = async () => {
      try {
        const bookshopResponse = await fetch(`${API_URL}/bookshops/${id}`);
        const bookshopData = await bookshopResponse.json();
        setBookshop(bookshopData);

        const salesResponse = await fetch(`${API_URL}/bookshops/${id}/sales`);
  ...
  const consignmentSales = sales.filter(sale => sale.payment_method === 'Consignment');
  ...
  return (
    <div>
      <Card title={`Bookshop: ${bookshop.name}`}>
        <p><strong>Location:</strong> {bookshop.location}</p>
        <p><strong>Contact:</strong> {bookshop.contact}</p>
        <p><strong>Consignment:</strong> {bookshop.consignment}</p>
      </Card>

      <Title level={4} style={{ marginTop: '24px' }}>Sales</Title>
      <Table
      ...
      {bookshop.consignment > 0 && (
        <>
          <Title level={4} style={{ marginTop: '24px' }}>Consignments</Title>
          <Table
            columns={salesColumns}
