...
import { Layout, Input, Card, Row, Col, Typography, Table, Button, message, Modal, Form, Select, InputNumber, Divider, Popover } from 'antd';
    ...
    return (
        <Modal title="Sale Successful" visible={visible} onCancel={onDone} footer={[
            ...
            <div ref={componentRef} style={{padding: 20}}>
                ...
                <Divider />
                <Table dataSource={sale.books} columns={[
                    ...
                    { title: 'Discount', dataIndex: ['SaleItem'], key: 'discount', render: (si) => si.discount > 0 ? `${si.discount} ${si.discount_type === 'Fixed' ? 'LKR' : '%'}` : '-'},
                ...
                <div style={{textAlign: 'right', marginTop: 16}}>
                    ...
                    <Text>Cart Discount: LKR {parseFloat(sale.discount).toFixed(2)}</Text><br/>
...
const ItemDiscountModal = ({ item, visible, onApply, onCancel }) => {
    ...
    return (
        <Modal title={`Discount for ${item.name}`} visible={visible} onOk={handleApply} onCancel={onCancel}>
...
const PosPage = () => {
    ...
    const handleAddToCart = (book) => {
        ...
        if (existing) {
            ...
            setCart([...cart, { ...book, quantity: 1, discountValue: 0, discountType: 'Fixed' }]);
        }
    };
    ...
    const handleItemDiscountApply = (bookId, discount) => {
        setCart(cart.map(item => item.id === bookId ? { ...item, ...discount } : item));
        setEditingItem(null);
    }
    ...
    const subtotalAfterItemDiscounts = cart.reduce((acc, item) => acc + calculateItemTotal(item), 0);
    const finalCartDiscount = cartDiscountType === 'Fixed' ? cartDiscountInput : (subtotalAfterItemDiscounts * cartDiscountInput) / 100;
    ...
    const cartColumns = [
        ...
        { title: 'Qty', dataIndex: 'quantity', key: 'quantity', width: 80, render: (text, record) => 
            <InputNumber size="small" min={0} value={text} onChange={(val) => handleQuantityChange(record.id, val)} />
        },
        { title: 'Discount', key: 'discount', render: (_, record) => (
            <Button icon={<EditOutlined />} size="small" onClick={() => setEditingItem(record)}>
                {record.discountValue > 0 ? `${record.discountValue}${record.discountType === 'Fixed' ? '' : '%'}` : 'Add'}
            </Button>
        )},
        { title: 'Subtotal', key: 'subtotal', render: (_, record) => `LKR ${calculateItemTotal(record).toFixed(2)}`}
    ];

    const cartDiscountSelector = <Select value={cartDiscountType} onChange={setCartDiscountType}><Option value="Fixed">LKR</Option><Option value="Percentage">%</Option></Select>;
            ...
            <Layout>
                ...
                <Sider width={450} theme="light" style={{ padding: '24px', borderLeft: '1px solid #f0f0f0' }}>
                    ...
                    <div style={{textAlign: 'right', marginTop: 24}}>
                        <Text strong>Subtotal: LKR {subtotal.toFixed(2)}</Text><br/>
                        <Text type="secondary">Subtotal (after item discounts): LKR {subtotalAfterItemDiscounts.toFixed(2)}</Text>
                        <div style={{display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 8}}>
                            <Text strong style={{marginRight: 8}}>Cart Discount:</Text>
                            <InputNumber value={cartDiscountInput} onChange={setCartDiscountInput} min={0} addonAfter={cartDiscountSelector} />
                        </div>
                        <Title level={4} style={{marginTop: 8}}>Total: LKR {total.toFixed(2)}</Title>
                    </div>
                    <Button type="primary" block size="large" disabled={cart.length === 0} onClick={() => setIsCheckoutVisible(true)} style={{ marginTop: 24 }}>Checkout</Button>
                </Sider>
            </Layout>
            {isCheckoutVisible && <CheckoutModal visible={isCheckoutVisible} onClose={() => setIsCheckoutVisible(false)} cart={cart} total={total} cartDiscount={{type: cartDiscountType, value: cartDiscountInput}} onSaleComplete={(sale) => { setIsCheckoutVisible(false); setCompletedSale(sale); }} />}
            {editingItem && <ItemDiscountModal item={editingItem} visible={!!editingItem} onApply={handleItemDiscountApply} onCancel={() => setEditingItem(null)} />}
            {completedSale && <Receipt sale={completedSale} visible={!!completedSale} onDone={resetSale} />}
        </Layout>
    );
}

const CheckoutModal = ({ visible, onClose, cart, total, cartDiscount, onSaleComplete }) => {
    const [form] = Form.useForm();
    ...
    const handleFinalizeSale = async () => {
        try {
          ...
          const saleData = { ...values, items: cart.map(item => ({ BookId: item.id, quantity: item.quantity, discount: item.discountValue, discount_type: item.discountType })), cartDiscount };
          ...
          if (response.ok) {
            ...
            throw new Error((await response.json()).message || 'Failed to create sale');
          }
    ...
    return (
        <Modal title="Finalize Sale" visible={visible} onOk={handleFinalizeSale} onCancel={onClose}>
            <Form form={form} layout="vertical"><PaymentForm form={form} total={total} bookshops={bookshops} /></Form>
        </Modal>
    )
}
        ...
        <>
            ...
            {paymentMethod === 'Cash' && (
                <Row gutter={16}>
                    ...
                    <Col span={12}><Title level={5} style={{paddingTop: 30}}>Change: LKR {change > 0 ? change.toFixed(2) : '0.00'}</Title></Col>
                </Row>
            )}
        </>
    )
